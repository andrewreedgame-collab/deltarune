...
"e52be439c3512d06a80bc8b89b9c65b9" ];
    ...
    function triggerAd(adId, _callback_beforeAd, _callback_afterAd, _callback_adDismissed, _callback_adViewed, _callback_adbreakDone) {
      ...
      function syncGameFilesystem(write, callback) {
        const FS = getEmscriptenFS();
        if (!FS || typeof FS.syncfs !== 'function') {
          Module.setStatus && Module.setStatus('Filesystem sync unavailable');
          console.warn('Emscripten FS.syncfs is not available');
          if (callback) callback(new Error('FS.syncfs unavailable'));
          return;
        }

        Module.setStatus(write ? 'Saving game...' : 'Loading game...');
        FS.syncfs(write, function (err) {
          if (err) {
            Module.setStatus(write ? 'Save failed' : 'Load failed');
            console.error('FS.syncfs error:', err);
          } else {
            Module.setStatus(write ? 'Game saved' : 'Game loaded');
            showRollbackMessage(write ? 'Save complete' : 'Load complete');
          }
      ...
      function saveGame() {
        syncGameFilesystem(true);
      }

      function loadGame() {
        syncGameFilesystem(false);
      }
